...
/* ================= 記録タブ(v5.14.0→v5.15.0→v5.21.0で刷新) =================
   ...
   機能は集約しつつ見やすく使いやすく」:
   構成: ①成長のヒーロー=覚えた単語・フレーズのバー(行のタップで📊学習のあゆみ/📊フレーズのあゆみ)
         ②覚えた数の伸び=週ごとの棒グラフ(8週・単語+フレーズ)
         ③タイル6枚=それぞれタップで詳細
           連続学習・学習した日→📅学習カレンダー(月ごとの色塗り・累計)/累計正解→📊学習のあゆみ(日ごとの解答数・正答率)/
           知識XP→🏆実績/今日の英語→📚読んだ・聴いたの記録/にがて→🔥にがてノート
  ...
  $("modal").querySelectorAll(".cd[data-k]").forEach(b=>{
    b.onclick=()=>{
      ...
      toast((+k.slice(5,7))+"/"+(+k.slice(8))+": "+(a
        ? a+"問(正解"+(r.c||0)+" ・ "+pctOf(r.c||0,a)+"%)"+(r.t? " ・ 目安"+r.t+(a>=r.t? " 達成🏅":"") : "")
...
function renderRecords(){
  ...
  const s=achSummary();
  box.innerHTML=
    // ① 成長のヒーロー: 全体に対してどこまで来たか+今週の伸び(行のタップであゆみ=v5.21.0)
    ...
    '<div class="panel statgrid" style="margin-top:12px">'+
      ...
      (GAME_ENABLED
        ...
        : statTile("📖 知識XP", "Lv"+accountLevel(), fmt(G.xp)+" XP ・ 🏆 実績 "+s.done+"/"+s.all, "statXP"))+
