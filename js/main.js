...
/* ---- 起動 ---- */
/* iOS: インストール後いちばん最初の起動だけ、ステータスバー領域が灰色のまま
   描かれることがある(実機確認。2回目以降の起動では起きない)。
   一度リロードすると直るため、ホーム画面起動の初回のみ自動でリロードする */
try{
  const standalone=navigator.standalone ||
    (window.matchMedia && matchMedia("(display-mode: standalone)").matches);
  if(standalone && !localStorage.getItem("tq_booted")){
    localStorage.setItem("tq_booted", "1"); // 書き込み失敗時はcatchへ=リロードしない
    location.reload();
  }
}catch(e){}
