...
const NEWS=[
  {d:"2026-09-26", t:"🔧 v5.29.1 実機FB2件: ①図鑑「単語」の詳細の「まだ出題されていない」を「未出題」に ②今日の英語の「✓ 読んだ」「✓ 聴いた」を間違えて押したとき、もう一度押す(「↩ 取り消す」)と取り消せるように(記録・今週の記録の📖🎧・実績の本数からも外れ、その回はまたおすすめに出る。取り消しは同期で他の端末にも伝わる)"},
  ...
  {d:"2026-09-25", t:"🧪 v5.28.0 Part 1模試に「穴埋め(本番形式)」: ①🧪の案内で「📋 25語の穴埋め問題をLLMに頼む」を押すと、25語入りの依頼文がコピーされる(本番と同じ空所補充・正解1+誤答3・原形で入る文) ②LLMの答え「英文( ) — 正解 — 誤答×3」を貼り戻すと問題セットになる(別の端末にも同期) ③「🧪 穴埋めではじめる」で英文の空所に入る語を4択で。経過時間・正解数・前回との比較は4択模試と同じで、解いた分はふつうの学習として記録。問題セットは1回解くと消える(次はまた作る)"},
  {d:"2026-09-25", t:"🧹 v5.27.0 学習の入口を整理: ①ホームの「🎯 フレーズ5問」を廃止(フレーズだけやりたいときは学習タブの「フレーズ」で。マイフレーズは期限が来た/未着手のものがミックス・フレーズの並べ替えに2回に1回優先して出る) ②実戦ドリル(グラフ描写・大人の動詞・無生物主語)を廃止。素材のフレーズはもともと内蔵フレーズの一部なので、ミックス/フレーズの並べ替えでそのまま学習できる ③学習タブの🎯ボタンは🧪になり、Part 1模試(25問・タイム)の案内と前回の結果を出してから始められる"},
  {d:"2026-09-25", t:"🔧 v5.26.2 📝メモのLLM依頼を実機の結果(Gemini)で確認して調整: ①例文は8〜12語・覚えたい表現以外はやさしい語で(13〜15語の長い文になっていた) ②3列目に「覚えたい表現」を返させ、それを核(🔑)にする(日本語のメモの核が finally・scheduled のような無関係な語になっていた) ③「ー」(長音)を区切りとして扱わないように(ドキュメンタリー等)"},
  {d:"2026-09-25", t:"🔧 v5.26.1 ①📝メモの英語(wipe out・portable のような単語・句動詞)は、LLMへの依頼を「その表現を含む会話で使える例文1文+日本語訳」に。貼り戻すと表現が核(🔑)になり、並べ替えで練習できる。短い語はチップで「フレーズ」にすればよい ②ホームの「30問=1セット」の脚注を削除(進行中のセットと達成の行だけ)"},
  {d:"2026-09-25", t:"📝 v5.26.0 メモを1か所に: ①ホームの🗣英会話パネルを「📝 分からなかった・言えなかった」に。1つの入力欄に英単語・英語の表現・日本語(言えなかったこと)を混ぜて書くと、行ごとに自動で仕分ける(単語=マイ単語に即登録・意味は自動取得/フレーズ=英訳待ちのメモ→LLMに頼んで貼り戻すとマイフレーズ)。仕分けは追加前のチップで変更でき、メモは✎で個別に書き直し・「→単語」で仕分け直しができる。学習タブの➕と今日の英語の📝もこの入口に ②ホームを整理: 「⏰◯時時点の目安」「次は◯セット目」の行と、英会話パネルの説明文を削除 ③実戦ドリルは全部選択式か並べ替えに。レッスン前ウォームアップ(口頭)とマイフレーズ特訓(4択)は「🎯 フレーズ5問」に統合=マイフレーズ優先に、まず自力で英文を思い出してから並べ替え ④日替わりの「今日のドリル」を廃止(セット完了画面から外した)。毎日の入口はホームの「🎯 フレーズ5問」"},
  {d:"2026-09-25", t:"🗣 v5.25.0 英会話の「言えなかった」を「言える」に: ①ホームに🗣英会話パネル。レッスン中・後に言えなかったことを日本語で1行メモ→📋依頼文をLLMに貼る→返ってきた「英文 — 日本語」を貼り戻すとマイフレーズに登録。翌日からミックスの5問目ごとのフレーズに優先して混ざる(2回に1回) ②🎤レッスン前ウォームアップ(約5分): マイフレーズ優先に5つ、日本語だけ見て声に出し、答えを見て⭕✖。レッスンで使えたものは「使えた」を押すと間隔をあけた正解として復習に反映 ③🧪Part 1模試: 本番と同じ25問(単語21+熟語4)を4択で、経過時間を表示。結果は前回と比べられる。🎯実戦メニューから"},
...
function renderHome(){
  ...
  $("homeBox").innerHTML=
    ...
    // ── 🗣英会話(v5.25.0・js/lesson.js): 言えなかったこと→英訳→マイフレーズ/ウォームアップ/使えた。今日の英語と同じ型
    sayPanelHTML()+
